...
diag_caja = function (datos, ind.columna, claves.a.mostrar = c()){
  ...
  ggboxplot = diag_caja_grafico_base(datos, ind.columna) + 
    ...
    geom_text(aes(label = nombres.filas), size = 3) 
...
biplot_2_colores2 = function (datos, 
                             claves.a.mostrar = c(), 
                             titulo = "",
                             titulo.grupo.a.mostrar = "Outliers",
                             colores = c("black", "red"),
                             porcentaje = 0.2) { # porcentaje = fracción de puntos a mostrar (0-1)
  
  # Submuestreo de los datos
  set.seed(123) # Para reproducibilidad
  n_total = nrow(datos)
  n_mostrar = round(n_total * porcentaje) # Cantidad de puntos a mostrar
  indices_mostrar = sample(1:n_total, n_mostrar) # Selección aleatoria de índices
  
  datos_submuestreados = datos[indices_mostrar, , drop = FALSE]
  claves.a.mostrar = claves.a.mostrar[claves.a.mostrar %in% indices_mostrar] # Ajustar claves
  
  nombres = rownames(datos_submuestreados)
  claves.datos = c(1:nrow(datos_submuestreados))
  son.a.mostrar = claves.datos %in% claves.a.mostrar
  nombres[!son.a.mostrar] = ''
  
  # PCA sobre los datos submuestreados
  PCA.model = princomp(scale(datos_submuestreados))
  outlier.shapes = c(".", "x") 
